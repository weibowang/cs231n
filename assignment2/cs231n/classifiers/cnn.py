...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        ############################################################################
        std = weight_scale
        self.params['W1'] = std * np.random.randn(num_filters, input_dim[0], filter_size, filter_size)
        self.params['b1'] = np.zeros(num_filters)
        
        # calculate the W2 size based on the info in the loss section
        # check https://github.com/cthorey/CS231/blob/master/assignment2/cs231n/classifiers/cnn.py
        stride_conv = 1  # stride
        P = (filter_size - 1) / 2  # padd
        Hc = (input_dim[1] + 2 * P - filter_size) / stride_conv + 1
        Wc = (input_dim[2] + 2 * P - filter_size) / stride_conv + 1
        width_pool = 2
        height_pool = 2
        stride_pool = 2
        ...
        self.params['W2'] = std * np.random.randn(num_filters * Hp * Wp, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        dW2 += self.reg * W2
        dout, dW1, db1 = conv_relu_pool_backward(dout, caches.pop())
        dW1 += self.reg * W1
        grads['W3'] = dW3
        grads['W2'] = dW2
        grads['W1'] = dW1
