...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    bW1 = np.append(W1, b1, axis=0)
    col = np.ones((X.shape[0], 1))
    bX = np.append(X, col, axis=1)
    f1 = bX.dot(bW1)
    f1 = np.maximum(0, f1)    # 5 * 10   

    b2 = b2.reshape(1, b2.shape[0])
    bW2 = np.append(W2, b2, axis=0)
    col = np.ones((f1.shape[0], 1))
    scores = np.append(f1, col, axis=1)
    scores = scores.dot(bW2) 
    ...
    #pass
    loss = 0.0
    # scores.shape 5 * 3   train_num * class
    ...
    # https://www.jianshu.com/p/fd34b6c68b4c
    dW2 = np.zeros([W2.shape[0], W2.shape[1]])
    scores = scores / scores_sum       # 5 * 3
    scores[np.arange(scores.shape[0]), y] -= 1
    scores /= scores.shape[0]
    dW2 = f1.T.dot(scores)       # 10 * 3
    dW2 += 2 * reg * W2
    grads['W2'] = dW2
    
    dB2 = np.zeros([1, b2.shape[0]])
    row = np.ones([1, f1.shape[0]])
    dB2 = row.dot(scores)
    dB2 = dB2.flatten()
    grads['b2'] = dB2 
        
    dW1 = np.zeros([W1.shape[0], W1.shape[1]])   #4 * 10
    t1 = scores.dot(W2.T)        # 5 * 10
    t1[(f1 <= 0)] = 0
    dW1 = X.T.dot(t1)
    dW1 += 2 * reg * W1
    grads['W1'] = dW1
    
    dB1 = np.zeros([1, b1.shape[0]])
    row = np.ones([1, t1.shape[0]])
    dB1 = row.dot(t1)
    dB1 = dB1.flatten()
    grads['b1'] = dB1
